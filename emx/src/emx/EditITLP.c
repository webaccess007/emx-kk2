...
void editITLP(HandlerType_t aHandlerType, uint8_t aPType, PtrUnion_t* aPtrUnion, uint8_t aY, uint8_t aColor)
{
    if (aHandlerType == HT_DISPLAY)
    {
        displayITLP(aHandlerType, aPType, aPtrUnion, aY, aColor);
        return;
    }

    // the value holder
    ITextListIdx_t   iTextListIdx;
    void *tgtAdr = pgmToRamITLP(aPType, aPtrUnion, &iTextListIdx);
